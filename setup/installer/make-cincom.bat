...
rem call mvn -f %SOURCE_HOME%\setup\couchdb\bjornloka package
...
set USER_INPUT_SPEC=%SCRIPT_HOME%\user-input-spec-cincom.xml
set USER_INPUT_SPEC_MODIFIED=%SCRIPT_HOME%\user-input-spec-cincom_modified.xml
...
call mvn -f %SOURCE_HOME%\nemakiware\pom-cincom.xml -Dmaven.test.skip=true package
rem call mvn -f %SOURCE_HOME%\nemakisolr clean
rem call mvn -f %SOURCE_HOME%\nemakisolr -Dmaven.test.skip=true package

rem Build installer
call %SCRIPT_HOME%\IzPack\bin\compile.bat %SCRIPT_HOME%\install-cincom.xml -b %SOURCE_HOME%\ -o %SCRIPT_HOME%\install.jar -k standard

rem Delete tmp file after putting them into installer
call mvn -f %SOURCE_HOME%\nemakiware clean
rem call mvn -f %SOURCE_HOME%\nemakisolr clean
call mvn -f %SCRIPT_HOME%\install-util clean
rem call mvn -f %SOURCE_HOME%\setup\couchdb\bjornloka clean
